function varargout = viewannotations(varargin)
% VIEWANNOTATIONS MATLAB code for viewannotations.fig
%      VIEWANNOTATIONS, by itself, creates a new VIEWANNOTATIONS or raises the existing
%      singleton*.
%
%      H = VIEWANNOTATIONS returns the handle to a new VIEWANNOTATIONS or the handle to
%      the existing singleton*.
%
%      VIEWANNOTATIONS('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in VIEWANNOTATIONS.M with the given input arguments.
%
%      VIEWANNOTATIONS('Property','Value',...) creates a new VIEWANNOTATIONS or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before viewannotations_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to viewannotations_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help viewannotations

% Last Modified by GUIDE v2.5 08-Jan-2016 10:53:53
...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @viewannotations_OpeningFcn, ...
                   'gui_OutputFcn',  @viewannotations_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before viewannotations is made visible.
function viewannotations_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to viewannotations (see VARARGIN)

% Choose default command line output for viewannotations
...
% UIWAIT makes viewannotations wait for user response (see UIRESUME)
% uiwait(handles.figure1);



% --- Outputs from this function are returned to the command line.
function varargout = viewannotations_OutputFcn(hObject, eventdata, handles) 
...
%        get(hObject,'Min') and get(hObject,'Max') to determine range of slider
    ...
    if ~isempty(find(all_labels, 1))
        contour(all_labels,'r');
    end
...
% handles    structure with handles and user data (see GUIDATA)
    ...
    if exist(labels_fpn,'file')
        load(labels_fpn);
        handles.label1=logical(label1);
        handles.label2=logical(label2);
        handles.label3=logical(label3);
        handles.label4=logical(label4);
        [l_ind]=check_annotated(label1+label2+label3+label4);
        handles.framenum=l_ind(end);
    else
       % if there is no labels, all labels set to zero 
       handles.label1=false(size(handles.Images));
       handles.label2=false(size(handles.Images));
       handles.label3=false(size(handles.Images));
       handles.label4=false(size(handles.Images));
       handles.framenum=1;
    end
    ...
    if ~isempty(find(all_labels, 1))
        contour(all_labels,'r');
        % update static text box2
        set(handles.text5, 'String',['Annotted:',num2str(numel(l_ind)),', Index:',num2str((l_ind))]);
    end

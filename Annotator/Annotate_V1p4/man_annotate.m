...
% Last Modified by GUIDE v2.5 06-Jan-2016 20:33:35
...
handles.label4=NaN;
handles.labels=NaN;
handles.metadata=NaN;
handles.person='unknown';
handles.device='Surfacepro';
handles.last_fnum=0;% last annoteted frame number
handles.FileName='';
if exist('datasetpath.txt','file')
   file_cnts=importdata('datasetpath.txt',' ');
   handles.PathName=file_cnts(1);
   handles.person=file_cnts(2);
   handles.device=file_cnts(3);    
   set(handles.popupmenu1,'Value',6);
else
    PathName=['C:\Users\michael\OneDrive\pacman\Interscalene_Dataset'];
    handles.PathName=PathName;
    fileID = fopen('datasetpath.txt','w');
    fprintf(fileID,'%s\n',PathName);
    fprintf(fileID,'%s\n','unknown');
    fprintf(fileID,'%s','unknown');
    fclose('all');
end
...
axes(handles.axes_lastannotate);
axis off
...
%        get(hObject,'Min') and get(hObject,'Max') to determine range of slider
    ...
    % update the last annotated if necessary
    if ~isempty(find(handles.labels(:,:,handles.framenum), 1))
        handles.last_fnum=handles.framenum;
        % display image and labels on the last annotated
        axes(handles.axes_lastannotate);
        imshow(handles.Images(:,:,handles.last_fnum));hold on;
        contour(handles.labels(:,:,handles.last_fnum),'r');
        set(handles.text_lastfn, 'String',[num2str(handles.framenum),'/',num2str(size(handles.Images,3))]);
    end
...
% handles    structure with handles and user data (see GUIDATA)
    ...
    if exist(labels_fpn,'file')
        ...
        handles.labels=label1+label2+label3+label4;
        handles.last_fnum=check_annotated(handles.labels);
        handles.framenum=handles.last_fnum;
        if exist('metadata','var')
            handles.metadata=metadata;
        else
            handles.metadata=struct;
        end
        clear labels1 label2 label3 label4;
    else
       ...
       handles.labels=zeros(size(handles.Images));
       handles.framenum=1;
       handles.metadata=struct;
    end
    ...
    % display labels
    axes(handles.axis_label);
    handles.labels(:,:,handles.framenum)=handles.label1(:,:,handles.framenum)+handles.label2(:,:,handles.framenum)+...
        handles.label3(:,:,handles.framenum)+handles.label4(:,:,handles.framenum);
    imshow(handles.labels(:,:,handles.framenum));
    
    % display image and labels on the last annotated
    if handles.last_fnum~=0
        axes(handles.axes_lastannotate);
        imshow(I1);hold on;
        contour(handles.labels(:,:,handles.framenum),'r');
    end
    ...
    % update static text box
    set(handles.text_lastfn, 'String',[num2str(handles.last_fnum),'/',num2str(size(handles.Images,3))]);
...
% store meta-data(time,annotator)
handles.metadata(handles.framenum).time=clock;
handles.metadata(handles.framenum).person=handles.person;
handles.metadata(handles.framenum).device=handles.device;
...
save(labels_fpn,'metadata','-append');
msgbox('Operation Completed','Success');

% save latest config into datasetpath.tx
if exist('datasetpath.txt','file')
    fileID = fopen('datasetpath.txt','w');
    fprintf(fileID,'%s\n',handles.PathName);
    fprintf(fileID,'%s\n',handles.person);
    fprintf(fileID,'%s',handles.device);
    fclose('all');
end
...
% handles    structure with handles and user data (see GUIDATA)
    ...
    if ~isempty(find(handles.labels(:,:,handles.framenum), 1))
        handles.last_fnum=handles.framenum;
        % display image and labels on the last-annotated axis
        axes(handles.axes_lastannotate);
        imshow(handles.Images(:,:,handles.last_fnum));hold on;
        contour(handles.labels(:,:,handles.last_fnum),'r');
        set(handles.text_lastfn, 'String',[num2str(handles.framenum),'/',num2str(size(handles.Images,3))]);
...
% return the lated annotated image
...
%        contents{get(hObject,'Value')} returns selected item from popupmenu1
    ...
    handles.person=contents{get(hObject,'Value')};
...
%        contents{get(hObject,'Value')} returns selected item from popupmenu2
    contents = cellstr(get(hObject,'String'));
    handles.device=contents{get(hObject,'Value')};
